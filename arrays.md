...
- [lla]() is a library for linear algebra, calling BLAS and LAPACK
...
Taking this approach further, domain specific languages have been built on Common
...
standard deviation of one, using the `alexandria` package:
...
distributions, there are packages available on Quicklisp:
...
The `aref` function takes the same number of index arguments as the
...
single flattened index via `row-major-aref`. The array size is given
by `array-total-size`, with the flattened index starting at 0.
...
to `each` in the `array-operations` library:
...
The LLA function `mm` performs vector-vector, matrix-vector and matrix-matrix
multiplication. 
